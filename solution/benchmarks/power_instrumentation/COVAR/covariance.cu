...
__global__ void mean_kernel(DATA_TYPE *mean, DATA_TYPE *data, size_t p_size)
{
...
__global__ void reduce_kernel(DATA_TYPE *mean, DATA_TYPE *data, size_t p_size)
{
	int j = blockIdx.x * blockDim.x + threadIdx.x + 1;
...
__global__ void covar_kernel(DATA_TYPE *symmat, DATA_TYPE *data, size_t p_size)
{
...
void covarianceCuda(DATA_TYPE* data, DATA_TYPE* symmat, DATA_TYPE* mean, DATA_TYPE* symmat_outputFromGpu)
{
	...
	PRE_KERNEL
	mean_kernel<<<grid1, block1>>>(mean_gpu,data_gpu, p_size);
	POST_KERNEL
	cudaThreadSynchronize();
	PRE_KERNEL
	reduce_kernel<<<grid2, block2>>>(mean_gpu,data_gpu, p_size);
	POST_KERNEL
	cudaThreadSynchronize();
	PRE_KERNEL
	covar_kernel<<<grid3, block3>>>(symmat_gpu,data_gpu, p_size);
	POST_KERNEL
	cudaThreadSynchronize();
	t_end = rtclock();
...
{
        bool check = true;
        if(sc::processArgs(argc, argv, p_size, check) >= 0) {
          if(p_size < 0) {
            if (p_size == -1)
              p_size = 2048;  // (2*N^2 + 2N) * sizeof(float) Byte = 8MB
            else if(p_size == -2)
              p_size = 4096;  // 32MB
            else if(p_size == -3)
              p_size = 8192; // 128MB
        ...
        fprintf(stdout, "Problemsize: %llu\n", p_size);
