...
__global__ void mvt_kernel1(DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1, size_t p_size)
{
...
__global__ void mvt_kernel2(DATA_TYPE *a, DATA_TYPE *x2, DATA_TYPE *y_2, size_t p_size)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
        size_t N = p_size;
			...
			DATA_TYPE* x1_outputFromGpu, DATA_TYPE* x2_outputFromGpu)
{
	...
	t_start = rtclock();
	PRE_KERNEL
	mvt_kernel1<<<grid,block>>>(a_gpu,x1_gpu,y_1_gpu, p_size);
	POST_KERNEL
	PRE_KERNEL
	mvt_kernel2<<<grid,block>>>(a_gpu,x2_gpu,y_2_gpu, p_size);
	POST_KERNEL
...
int main(int argc, char *argv[])
{
        ...
        if(sc::processArgs(argc, argv, p_size, check) >= 0) {
          if(p_size < 0) {
            if (p_size == -1)
              p_size = 4096;  // 4096^2 * sizeof(float) * 2 Byte = 128MB
            else if(p_size == -2)
              p_size = 8192;  // 512MB
            else if(p_size == -3)
              p_size = 16374; // 2GB
