...
__global__ void atax_kernel1(DATA_TYPE *A, DATA_TYPE *x, DATA_TYPE *tmp, size_t p_size)
{
...
__global__ void atax_kernel2(DATA_TYPE *A, DATA_TYPE *y, DATA_TYPE *tmp, size_t p_size)
{
...
void ataxGpu(DATA_TYPE* A, DATA_TYPE* x, DATA_TYPE* y, DATA_TYPE* tmp, DATA_TYPE* y_outputFromGpu)
{
	...
	atax_kernel1<<< grid1, block >>>(A_gpu,x_gpu,tmp_gpu, p_size);
	
	cudaThreadSynchronize();
	
	atax_kernel2<<< grid2, block >>>(A_gpu,y_gpu,tmp_gpu, p_size);
...
int main(int argc, char** argv)
{
        ...
        if(sc::processArgs(argc, argv, p_size, check) >= 0) {
          if(p_size < 0) {
            if (p_size == -1)
              p_size = 4096;  // (N^2 + 3N) * sizeof(float) Byte = 64MB
            else if(p_size == -2)
              p_size = 8192;  // 128MB
            else if(p_size == -3)
              p_size = 16384; // 1GB
