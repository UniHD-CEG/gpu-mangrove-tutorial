...
__global__ void gemm_kernel(DATA_TYPE *a, DATA_TYPE *b, DATA_TYPE *c, size_t p_size)
{
...
void gemmCuda(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* C, DATA_TYPE* C_outputFromGpu)
{
	...
	PRE_KERNEL
	gemm_kernel<<< grid, block >>>(A_gpu, B_gpu, C_gpu, p_size);
	POST_KERNEL
...
int main(int argc, char *argv[])
{
        ...
        if(sc::processArgs(argc, argv, p_size, check) >= 0) {
          if(p_size < 0) {
            if (p_size == -1)
              p_size = 512;  // 4096^2 * sizeof(float) * 2 Byte = 128MB
            else if(p_size == -2)
              p_size = 1024;  // 512MB
            else if(p_size == -3)
              p_size = 2048; // 2GB
