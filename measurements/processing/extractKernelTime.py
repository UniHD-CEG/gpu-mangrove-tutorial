...
def main():
    ...
    cur_mem.execute('''create table kerneltime
                    (bench text not null,
                    app text not null,
    ...
    for bench,app,data,build in key_set:
        ...
        for item in res:
            print(item)
            try:
                for line in open(os.path.join(args.logdir,'../',item[0],'kerneltime.csv'), 'r'):
                    line = line.split(',')
                    try:
                        ...
                        continue
                    tmp = cur_mem.execute("insert into kerneltime values(?,?,?,?,?,?)", (bench, app, data,build, launch_seq, time,)) 
            except IOError:
                print("\033[1;31mIOError!\033[0m")
                continue
            conn_mem.commit()

    tmp = cur_mem.execute("select  bench,app,dataset,build,lseq,avg(time) from kerneltime group by bench,app,dataset,build,lseq")
